...
exports.up = (pgm) => {
  ...
  pgm.createIndex("applications", ["company_id"], {
    name: "idx_applications_company_id",
  });
  pgm.createIndex("applications", ["user_id"], {
    name: "idx_applications_user_id",
  });
  pgm.createIndex("applications", ["date_submitted"], {
    name: "idx_applications_date_submitted",
  });
  ...
  pgm.sql(
    "CREATE INDEX IF NOT EXISTS idx_applications_user_id_date_submitted ON applications (user_id, date_submitted DESC);",
  );

  pgm.createIndex("resumes", ["user_id"], { name: "idx_resumes_user_id" });
  pgm.createIndex("resumes", ["application_id"], {
    name: "idx_resumes_application_id",
  });
  pgm.createIndex("resumes", ["created_at"], {
  ...
  pgm.createIndex("cover_experiences", ["user_id"], {
    name: "idx_cover_experiences_user_id",
  });
  pgm.createIndex("resume_experiences", ["user_id"], {
    name: "idx_resume_experiences_user_id",
  });

  pgm.createIndex("resume_lines", ["resume_id"], {
    name: "idx_resume_lines_resume_id",
  });

  pgm.createIndex("user_skills", ["user_id"], {
    name: "idx_user_skills_user_id",
  });
  pgm.createIndex("user_work_experience", ["user_id"], {
    name: "idx_user_work_experience_user_id",
  });
  pgm.createIndex("user_education", ["user_id"], {
    name: "idx_user_education_user_id",
  });

  // Partial index for applications that are not complete (helps queries like WHERE is_complete = 'false')
  pgm.sql(
    "CREATE INDEX IF NOT EXISTS idx_applications_incomplete ON applications (user_id) WHERE is_complete = 'false';",
  );

  // Full-text search index on application posting text (combines posting_text and analyzed_posting_text)
  pgm.sql(
    `CREATE INDEX IF NOT EXISTS idx_applications_posting_text_tsv ON applications USING GIN (to_tsvector('english', coalesce(posting_text,'') || ' ' || coalesce(analyzed_posting_text,'')));`,
  );
  ...
  pgm.sql(
    "CREATE INDEX IF NOT EXISTS idx_applications_posting_url_trgm ON applications USING gin (posting_url gin_trgm_ops);",
  );
  pgm.sql(
  ...
  pgm.createIndex("applications", ["is_complete"], {
    name: "idx_applications_is_complete",
  });
};
...
exports.down = (pgm) => {
  ...
  pgm.dropIndex("applications", "idx_applications_company_id", {
    ifExists: true,
  });
  pgm.dropIndex("applications", "idx_applications_user_id", { ifExists: true });
  pgm.dropIndex("applications", "idx_applications_date_submitted", {
    ifExists: true,
  });
  pgm.dropIndex("applications", "idx_applications_created_at", {
    ifExists: true,
  });
  pgm.dropIndex("applications", "idx_applications_user_id_date_submitted", {
    ifExists: true,
  });
  pgm.dropIndex("applications", "idx_applications_is_complete", {
    ifExists: true,
  });
  ...
  pgm.dropIndex("cover_experiences", "idx_cover_experiences_user_id", {
    ifExists: true,
  });
  pgm.dropIndex("resume_experiences", "idx_resume_experiences_user_id", {
    ifExists: true,
  });
  pgm.dropIndex("resume_lines", "idx_resume_lines_resume_id", {
    ifExists: true,
  });

  pgm.dropIndex("user_skills", "idx_user_skills_user_id", { ifExists: true });
  pgm.dropIndex("user_work_experience", "idx_user_work_experience_user_id", {
    ifExists: true,
  });
  pgm.dropIndex("user_education", "idx_user_education_user_id", {
    ifExists: true,
  });

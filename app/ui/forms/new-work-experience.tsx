// definition: Add a new work experience entry for the authenticated user.
// file: app/(dashboard)/dashboard/work-experience/new/page.tsx
...
export default function NewWorkExperience({ user }: Props) {
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = (await createWorkExperience(formData)) as any;
      ...
      if (result?.errors) {
        if (typeof result.errors === "object") {
          ...
          setStatusMessage(
            "Failed to create work experience. Please try again.",
          );
          console.error("createWorkExperience failed:", result);
        }
        return;
      }

      setEdited(false);
      setStatusMessage("Work experience created successfully!");

      // Reset form
      formRef.current?.reset();

      // Redirect to work experience list after short delay
      setTimeout(() => {
        startTransition(() => {
  ...
  return (
    <div className="overflow-y-auto w-full max-w-4xl px-4 sm:px-6 pb-8">
      ...
      <form
        ...
        className="new-work-experience-container new-work-experience-form space-y-6 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm"
        noValidate
      >
        <input
          readOnly

 ...
 *
 * app/components/__tests__/CoverLetterIdError.test.tsx
 *
 * Tests for the client component CoverLetterIdError
 *
...
import CoverLetterIdError from "../error";

describe("CoverLetterIdError (client)", () => {
  ...
  it("logs the error to console on mount and renders ErrorFallback with props", async () => {
    ...
    render(<CoverLetterIdError error={err} reset={reset} />);

    // Wait for useEffect to run and call console.error
    await waitFor(() => {
      expect(console.error).toHaveBeenCalledWith("[CoverLetterIdError]", err);
    });
  ...
  it("still renders when error is present and reset is callable", async () => {
    const err = new Error("failed to load cover letter details");
    const reset = jest.fn();

 ...
 *
 * app/components/__tests__/CoverLetterError.test.tsx
 *
 * Tests for the client component CoverLetterError
 *
...
import CoverLetterError from "../error";

describe("CoverLetterError (client)", () => {
  ...
  it("logs the error to console on mount and renders ErrorFallback with props", async () => {
    const err = new Error("cover letter fetch failed");
    const reset = jest.fn();

    render(<CoverLetterError error={err} reset={reset} />);

    // Wait for useEffect to run and call console.error
    await waitFor(() => {
      expect(console.error).toHaveBeenCalledWith("[CoverLetterError]", err);
    });

    // Mocked ErrorFallback should render the error message
    expect(screen.getByTestId("mock-error-message")).toHaveTextContent(
      "cover letter fetch failed",
    );
  ...
  it("still renders when error is present and reset is callable", async () => {
    const err = new Error("another cover letter error");
    const reset = jest.fn();

 ...
 *
 * app/dashboard/skills/__tests__/error.test.tsx
 *
 * Tests for the client component app/dashboard/skills/error.tsx
 *
...
describe("Skills RouteError (client)", () => {
  ...
  it("logs the error to console on mount and renders ErrorFallback with props", async () => {
    const err = new Error("Failed to fetch skills");
    (err as any).digest = "digest-skills-002"; // optional digest similar to Next.js
    const reset = jest.fn();
  ...
  it("still renders when error has no digest", async () => {
    const err = new Error("Generic skills error");
    const reset = jest.fn();

 ...
 *
 * app/components/__tests__/DashboardOverviewError.test.tsx
 *
 * Tests for the client component DashboardOverviewError
 *
...
// Import after mocking. Adjust the relative path if needed.
import DashboardOverviewError from "../error";

describe("DashboardOverviewError (client)", () => {
  ...
  it("logs the error to console on mount and renders ErrorFallback with props", async () => {
    const err = new Error("overview load failed");
    const reset = jest.fn();

    render(<DashboardOverviewError error={err} reset={reset} />);
  ...
  it("still renders when error is present and reset is callable", async () => {
    const err = new Error("another overview error");
    const reset = jest.fn();
